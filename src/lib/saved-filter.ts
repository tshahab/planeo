...
const allowed = ["q", "query", "queryVersion", "project", "type", "status", "assignee", "reporter", "priority", "label", "sprint", "release", "requestType", "from", "to", "sort"] as const;
...
export function normalizeSavedQuery(value: unknown): SavedQuery | null {
  ...
  if (result.query) {
    if (result.queryVersion && result.queryVersion !== "1") return null;
    try { parseAdvancedQuery(result.query); } catch { return null; }
    result.queryVersion = "1";
  }
  return result;
}
