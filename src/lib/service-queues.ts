...
export const QUEUE_COLUMNS = ["summary", "status", "priority", "assignee", "requestType", "organization", "createdAt", "updatedAt", "slaState", "slaGoal", "slaRemaining", "customerResponse"] as const;
...
export type QueueRow = { id: string; requestId: string; version: number; summary: string; number: number; status: string; statusId: string; priority: string; assignee: string; assigneeId: string | null; requestType: string; organization: string; createdAt: string; updatedAt: string; slaState: string; slaGoal?: string; slaRemaining?: string; customerResponse: string | null };
...
export async function createQueueSnapshot(context: AuthContext, key: string, queueId: string) {
  ...
  return db.$transaction(async tx => {
    ...
    const issues = await tx.issue.findMany({ where, take: 2001, orderBy: [{ [definition.sort]: definition.direction }, { id: "asc" }], select: { id: true, number: true, summary: true, version: true, statusId: true, priority: true, assigneeId: true, createdAt: true, updatedAt: true, status: { select: { name: true } }, assignee: { select: { name: true } }, serviceRequest: { select: { id: true, slaState: true, requestType: { select: { name: true } }, customerOrganization: { select: { name: true } }, portalComments: { where: { customerId: { not: null } }, orderBy: { createdAt: "desc" }, take: 1, select: { createdAt: true } } } } } });
    if (issues.length > 2000) throw new QueueError("Refine this queue to at most 2,000 requests before taking a snapshot.");
    const cycles = await tx.slaCycle.findMany({ where: { requestId: { in: issues.map(issue => issue.serviceRequest!.id) } }, distinct: ["requestId", "metric"], orderBy: { sequence: "desc" }, select: { requestId: true, metric: true, elapsedMs: true, goalVersion: { select: { targetMinutes: true, goal: { select: { name: true } } } } } });
    ...
    for (const row of rows) { const targets = cycles.filter(cycle => cycle.requestId === row.requestId); row.slaGoal = targets.map(cycle => cycle.goalVersion.goal.name).join("; ") || "None"; row.slaRemaining = targets.map(cycle => `${cycle.metric}: ${Math.ceil(Math.max(0, cycle.goalVersion.targetMinutes * 60000 - Number(cycle.elapsedMs)) / 60000)} business minutes`).join("; ") || "—"; }
