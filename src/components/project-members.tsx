...
export function ProjectMembers({ projectKey, projectName, onClose }: { projectKey: string; projectName: string; onClose: () => void }) {
  ...
  async function invite(event: React.FormEvent) {
    ...
    try {
      ...
      const result = await response.json() as { invitation?: { acceptPath?: string }; error?: string };
      if (!response.ok) throw new Error(result.error ?? "The member could not be added.");
      if (result.invitation?.acceptPath) setInviteLink(`${window.location.origin}${result.invitation.acceptPath}`);
      setEmail("");
      await load();
    } catch (cause) {
