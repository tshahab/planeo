...
export function Backlog({ projectKey, canWrite, onSelect }: { projectKey: string; canWrite: boolean; onSelect: (issue: Issue) => void }) {
  ...
  return <section className="backlog-view">
    ...
    {active && <SprintSection sprint={active} canWrite={canWrite} backlog={backlog} onSelect={onSelect} onAction={(body) => request(sprintUrl(active.id), body).catch((reason: Error) => setError(reason.message))} />}
    {planned.map((sprint) => <SprintSection key={sprint.id} sprint={sprint} canWrite={canWrite} backlog={backlog} onSelect={onSelect} onAction={(body) => request(sprintUrl(sprint.id), body).catch((reason: Error) => setError(reason.message))} />)}
    <section className="sprint-section"><header><div><h3>Backlog</h3><span>{backlog.length} issues</span></div></header><div className="backlog-list">{backlog.map((issue, index) => <div className="backlog-row" key={issue.id}><div className="order-buttons">{canWrite && <><button onClick={() => moveBacklog(index, -1)} disabled={index === 0} aria-label="Move up"><ArrowUp size={14}/></button><button onClick={() => moveBacklog(index, 1)} disabled={index === backlog.length - 1} aria-label="Move down"><ArrowDown size={14}/></button></>}</div><button className="issue-link" onClick={() => onSelect(issue)}><strong>{issue.key}</strong><span>{issue.title}</span><em>{issue.priority}</em></button>{canWrite && planned.length > 0 && <select aria-label={`Assign ${issue.key} to sprint`} defaultValue="" onChange={(event) => { if (event.target.value) request(sprintUrl(event.target.value), { action: "add", issueId: issue.id }).catch((reason: Error) => setError(reason.message)); }}><option value="">Add to sprint…</option>{planned.map((sprint) => <option key={sprint.id} value={sprint.id}>{sprint.name}</option>)}</select>}</div>)}{backlog.length === 0 && <p className="empty-backlog">Everything is planned.</p>}</div></section>
...
function SprintSection({ sprint, backlog, canWrite, onSelect, onAction }: { sprint: SprintSummary; backlog: Issue[]; canWrite: boolean; onSelect: (issue: Issue) => void; onAction: (body: object) => Promise<void> }) {
  ...
  return <section className={`sprint-section ${sprint.state === "ACTIVE" ? "active-sprint" : ""}`}><header><div><h3>{sprint.name}</h3><span>{sprint.state === "ACTIVE" ? `${done}/${sprint.issues.length} done` : `${sprint.issues.length} issues`}</span></div>{canWrite && <div>{sprint.state === "PLANNED" && <button className="secondary-button" onClick={() => onAction({ action: "start" })}><Play size={15}/> Start sprint</button>}{sprint.state === "ACTIVE" && <button className="secondary-button" onClick={() => onAction({ action: "complete" })}><CheckCircle2 size={15}/> Complete sprint</button>}</div>}</header>{sprint.state === "ACTIVE" && <div className="progress-track"><span style={{ width: `${sprint.issues.length ? done / sprint.issues.length * 100 : 0}%` }}/></div>}<div className="backlog-list">{sprint.issues.map((issue) => <div className="backlog-row" key={issue.id}><button className="issue-link" onClick={() => onSelect(issue)}><strong>{issue.key}</strong><span>{issue.title}</span><em>{issue.status}</em></button>{canWrite && <button className="remove-sprint" onClick={() => onAction({ action: "remove", issueId: issue.id })}><RotateCcw size={14}/> Backlog</button>}</div>)}{sprint.issues.length === 0 && <p className="empty-backlog">Add issues from the backlog below.</p>}</div>{canWrite && backlog.length > 0 && sprint.state === "ACTIVE" && <select className="active-add" defaultValue="" onChange={(event) => { if (event.target.value) onAction({ action: "add", issueId: event.target.value }); }}><option value="">Add an issue…</option>{backlog.map((issue) => <option key={issue.id} value={issue.id}>{issue.key} · {issue.title}</option>)}</select>}</section>;
}
