...
import { columns, people } from "@/lib/demo-data";
import type { Issue, Person, Status } from "@/lib/types";
import { Avatar } from "./workspace-app";

export function IssuePanel({ issue, onClose, onMove, onUpdate, readOnly = false }: { issue: Issue; onClose: () => void; onMove: (status: Status) => void; onUpdate: (changes: Record<string, unknown>) => Promise<void>; readOnly?: boolean }) {
      ...
      <aside className="issue-panel" aria-label={`${issue.key} details`}>
        ...
        <div className="panel-body">
          <main className="issue-main">
            <div className="issue-type-line"><span className={`type-pill ${issue.type.toLowerCase()}`}>{issue.type}</span><span>Created 2 days ago</span></div>
          ...
          <aside className="issue-properties">
            <label>Status</label><select value={issue.status} disabled={readOnly} onChange={(event) => onMove(event.target.value as Status)}>{columns.map((column) => <option key={column.status}>{column.status}</option>)}</select>
            ...
            <label>Sprint</label><button className="property-value"><Clock3 size={16} />Sprint 8</button>
            ...
            <p className="property-small">Reporter <strong>Mina Park</strong></p><p className="property-small">Last updated <strong>18 minutes ago</strong></p>
          </aside>
        </div>
      </aside>
    </div>
