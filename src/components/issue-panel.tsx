...
export function IssuePanel({ issue, statuses, currentUser, onClose, onMove, onUpdate, onArchive, readOnly = false }: { issue: Issue; statuses: ProjectStatus[]; currentUser: Person; onClose: () => void; onMove: (status: Status) => void; onUpdate: (changes: Record<string, unknown>) => Promise<void>; onArchive?: () => Promise<void>; readOnly?: boolean }) {
  ...
  const [development, setDevelopment] = useState<Array<{id:string;kind:string;title:string;url:string;authorName:string|null;targetBranch:string|null;state:string|null;reviewState:string|null;buildState:string|null;repository:{fullName:string;accessible:boolean;connection:{provider:string}}}>>([]);

  useEffect(() => {
    const controller = new AbortController();
    fetch(`/api/issues/${issue.id}/details`, { signal: controller.signal })
      .then(async (response) => {
        const result = await response.json() as { comments?: typeof comments; activities?: typeof activities; attachments?: typeof attachments; subtasks?: Issue[]; development?: typeof development; error?: string };
        if (!response.ok) throw new Error(result.error ?? "Issue activity could not be loaded.");
        return result;
      })
      .then((result) => { setComments(result.comments ?? []); setActivities(result.activities ?? []); setAttachments(result.attachments ?? []); setSubtasks(result.subtasks ?? []); setDevelopment(result.development ?? []); })
      ...
      <aside className="issue-panel" aria-label={`${issue.key} details`}>
        ...
        <div className="panel-body">
          <main className="issue-main">
            ...
            {development.length > 0 && <section className="activity-section" aria-labelledby="development-heading"><h3 id="development-heading">Development</h3>{development.map((item) => <a className="activity-item" key={item.id} href={item.url} target="_blank" rel="noreferrer"><div><p><strong>{item.repository.connection.provider} · {item.repository.fullName}</strong> {item.kind.replaceAll("_", " ").toLowerCase()}</p><blockquote>{item.title}</blockquote><span>{[item.authorName, item.targetBranch, item.reviewState ?? item.buildState ?? item.state, !item.repository.accessible ? "Repository unavailable" : null].filter(Boolean).join(" · ")}</span></div></a>)}</section>}
          </main>
          <aside className="issue-properties">
            ...
            <label>Releases</label><div className="property-labels">{issue.releases?.length?issue.releases.map(release=><span key={release.id}>{release.name}{release.archived?" (archived)":""}</span>):<span>None</span>}</div>
            {Object.entries(issue.customFields ?? {}).map(([fieldId, field]) => <div className="custom-field-property" key={fieldId}><label htmlFor={`custom-${fieldId}`}>{field.name}{field.archived ? " (archived)" : ""}</label>{field.type === "BOOLEAN" ? <input id={`custom-${fieldId}`} type="checkbox" checked={field.value === true} disabled={readOnly || field.archived} onChange={(event) => onUpdate({ customFields: { [fieldId]: event.target.checked } }).catch((cause) => setActivityError(cause instanceof Error ? cause.message : "Custom field could not be saved."))} /> : field.type === "SINGLE_SELECT" ? <select id={`custom-${fieldId}`} value={String(field.value ?? "")} disabled={readOnly || field.archived} onChange={(event) => onUpdate({ customFields: { [fieldId]: event.target.value } }).catch((cause) => setActivityError(cause instanceof Error ? cause.message : "Custom field could not be saved."))}>{field.options.map((option) => <option key={option}>{option}</option>)}</select> : <input id={`custom-${fieldId}`} type={field.type === "NUMBER" ? "number" : field.type === "DATE" ? "date" : field.type === "URL" ? "url" : "text"} value={String(field.value ?? "")} disabled={readOnly || field.archived} onChange={(event) => { const value = field.type === "NUMBER" ? Number(event.target.value) : event.target.value; void onUpdate({ customFields: { [fieldId]: value } }).catch((cause) => setActivityError(cause instanceof Error ? cause.message : "Custom field could not be saved.")); }} />}</div>)}
