...
export async function POST(request: Request) {
  ...
  try { issue = await db.$transaction(async (tx) => {
    ...
    await publishRealtime(tx, { workspaceId: project.workspaceId, projectId: project.id, type: "issue.created", resourceId: created.id, payload: { id: created.id, version: created.version } });
