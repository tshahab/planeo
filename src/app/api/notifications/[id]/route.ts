...
export async function PATCH(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const notification = await db.notification.findFirst({ where: { id, workspaceId: context.workspace.id, userId: context.user.id, issue: { is: { archivedAt: null, project: { is: accessibleProjectWhere(context) }, AND: [await issueSecurityWhere(context)] } } } });
