...
export async function GET(request: Request) {
  ...
  const where = { workspaceId: context.workspace.id, userId: context.user.id, issue: { is: { archivedAt: null, project: { is: accessibleProjectWhere(context) }, AND: [await issueSecurityWhere(context)] } } };
