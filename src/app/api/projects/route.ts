...
export async function POST(request: Request) {
  ...
  const project = await db.$transaction(async (tx) => {
    ...
    const typeData = template === "SERVICE" ? [["Service request", "TASK"]] as const : [["Epic", "EPIC"], ["Story", "STORY"], ["Task", "TASK"], ["Bug", "BUG"], ["Subtask", "SUBTASK"]] as const;
    ...
    if (template === "SERVICE") await tx.portalGroup.create({ data: { projectId: created.id, name: "Get help", description: "Contact the service team", position: 0 } });
