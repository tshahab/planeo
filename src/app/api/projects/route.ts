...
export async function POST(request: Request) {
  ...
  const template = body?.template === "SCRUM" || body?.template === "SERVICE" ? body.template : "KANBAN";
  ...
  const project = await db.$transaction(async (tx) => {
    ...
    const typeData = template === "SERVICE" ? [["Service request", "TASK"], ["Incident", "BUG"], ["Change", "TASK"]] as const : [["Epic", "EPIC"], ["Story", "STORY"], ["Task", "TASK"], ["Bug", "BUG"], ["Subtask", "SUBTASK"]] as const;
    ...
    const statusData = template === "SERVICE"
      ? [["Waiting for support", "TODO", "#8a93a3"], ["In progress", "IN_PROGRESS", "#5a72d8"], ["Waiting for customer", "IN_PROGRESS", "#a16bc0"], ["Resolved", "DONE", "#43a47e"]] as const
      : template === "SCRUM"
    ...
    if (template === "SERVICE") await tx.portalGroup.create({ data: { projectId: created.id, name: "Get help", description: "Request help from the service team." } });
