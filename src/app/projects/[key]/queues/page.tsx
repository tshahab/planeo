...
export default async function QueuesPage({ params }: { params: Promise<{ key: string }> }) {
  ...
  return <main className="portal-page"><header><strong>{project.name}</strong><nav><Link href={`/projects/${project.key}`}>Project overview</Link><Link href={`/projects/${project.key}/reports/sla`}>SLA report</Link>{canAdmin && <Link href={`/projects/${project.key}/settings/sla`}>SLA settings</Link>}{canAdmin && <Link href={`/projects/${project.key}/settings/mailbox`}>Service email</Link>}</nav></header><h1>Agent queues</h1><ServiceQueues projectKey={project.key} userId={context.user.id} canAdmin={canAdmin} canEdit={canEdit} options={{ statuses, requestTypes, people: people.map(item => item.user), organizations: organizations.map(item => item.organization), labels }} /></main>;
}
