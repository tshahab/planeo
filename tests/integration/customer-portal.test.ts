...
describe("customer portal authorization", () => {
  ...
  it("serializes only authorized public realtime signals", async () => {
    ...
    const body = await response.json(); expect(body.events).toHaveLength(1); expect(typeof body.events[0].id).toBe("string"); expect(body.events[0].type).toBe("request.changed"); expect(body.events[0].payload).toBeUndefined();
  });
